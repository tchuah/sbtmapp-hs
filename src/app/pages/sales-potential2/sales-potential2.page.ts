...
@Component({
  selector: 'app-sales-potential2',
  templateUrl: './sales-potential2.page.html',
  styleUrls: ['./sales-potential2.page.scss'],
})
export class SalesPotential2Page implements OnInit, OnDestroy {
  ...
  async resetCollection() {
    this.util.msgboxConfirmXTranslate('Client Alert!', 'Are you sure to revert collection?', async () => {
      const loading = await this.loadingController.create({
        message: 'Please wait...'
      });
      await loading.present();

      this.api.httpAction(`Main/ACT_FunctionAmendSales?Id=${this.paramId}&KeyAction=RESET-COLLECTION&JsonParam=${this.currentField}`, {})
        .pipe(takeUntil(this.destroy$))
        .subscribe(async data => {
          if (data[0].ReturnCode === 'OK') {
            loading.dismiss();
